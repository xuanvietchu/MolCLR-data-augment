...
FORMULA_REGEX = re.compile(
    '(' + '|'.join(
        list(ABBREVIATIONS.keys()) +  # 1. Match abbreviations (e.g., NO2, OMe)
        ['R[0-9]*',                   # 2. Match R-groups (e.g., R, R1)
        '[A-Z][a-z]+',                # 3. Two-letter elements (e.g., Cl, Br)
        '[A-Z]',                       # 4. Single-letter elements (e.g., C, O)
        '[0-9]+',                      # 5. Numbers (e.g., 2 in H2O)
        '\(|\)']                       # 6. Parentheses
    ) + ')'
)
