...
class MoleculeDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        def expand_mask(mask_nodes, expansion_prob):
            expanded_set = set(mask_nodes)
            for node in mask_nodes:
                if node in atom_to_group and random.random() < atom_to_group[node][1]:
                    if expansion_prob < 1/3:
                        expanded_set.update(atom_to_group[node][0])
                    elif expansion_prob < 2/3:
                        expanded_set.difference_update(atom_to_group[node][0])
            return list(expanded_set)

        mask_nodes_i = expand_mask(mask_nodes_i, random.random())
        mask_nodes_j = expand_mask(mask_nodes_j, random.random())
